...
// Main widget for the level four scene section.
class LevelFourScene extends StatefulWidget {
  const LevelFourScene({super.key});

  @override
  // Creates the state object for this widget.
  State<LevelFourScene> createState() => _LevelFourSceneState();
}

// State for the level four scene widget.
class _LevelFourSceneState extends State<LevelFourScene> {
  late final LevelFourGame game;
  ...
  Future<void> _restartLevel() async {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(builder: (_) => const LevelFourScene()),
    );
  }
        ...
        children: [
          Positioned.fill(
            child: GameWidget(
              game: game,
              overlayBuilderMap: {
                'LevelHint': (context, LevelFourGame game) =>
                    _buildLevelHint(game),
                'SuperPowerHud': (context, LevelFourGame game) =>
                    _buildSuperPowerHud(game),
                'VictoryMenu': (context, LevelFourGame game) => Center(
                  child: Container(
                    width: 360,
                ...
                'DefeatMenu': (context, LevelFourGame game) => Center(
                  child: Container(
                    width: 360,
                    padding: const EdgeInsets.all(30),
                    decoration: BoxDecoration(
  ...
  Widget _buildLevelHint(LevelFourGame game) {
  ...
  Widget _buildPigKillHud(LevelFourGame game) {
    return IgnorePointer(
  ...
  Widget _buildSuperPowerHud(LevelFourGame game) {
    return IgnorePointer(
      child: SafeArea(
        child: Align(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            ListTile(
                              ...
                              splashColor: Colors.white24,
                              onTap: () {
                                LevelProgress.clearCheckpointForLevel(4);
                                Navigator.pop(context);
                                _restartLevel();
                              },
                            ),
...
// Main game logic for the level four game scene.
class LevelFourGame extends FlameGame with HasCollisionDetection {
  ...
  late PlayerFour player;
  EndDoorFour? endDoor;
  List<Rect> groundRects = [];
  double mapWidth = 0;
  double mapHeight = 0;
  int pigKills = 0;
  final int requiredPigKills = 25;
  ...
  Future<void> onLoad() async {
    ...
    if (spawns != null) {
      ...
      final useResume = LevelProgress.consumeResumeChoiceForLaunch(level: 4);
      final resume = LevelProgress.resumePositionForLevel(4);
      if (useResume && resume != null) {
        player.position.setValues(resume.x, resume.y);
        showLevelHint('Resumed from your last checkpoint.');
      }
      LevelProgress.setLastPlayedLevel(4);
      ...
      for (final obj in spawns.objects) {
        ...
        if (objName == 'startdoor') {
          ...
          endDoor = EndDoorFour(position: Vector2(obj.x, obj.y));
          world.add(endDoor!);
          ...
          world.add(HeartPickupFour(position: Vector2(obj.x, obj.y)));
...
// Game component for the player four object.
class PlayerFour extends SpriteAnimationComponent
    with HasGameRef<LevelFourGame>, CollisionCallbacks {
  ...
  // Handles enter door.
  Future<void> enterDoor(EndDoorFour door) async {
    if (isDead || isEntering || isEnteringDoor) return;
    isEnteringDoor = true;
    horizontalMovement = 0;
    velocity = Vector2.zero();
    position.setValues(
      door.position.x,
      door.position.y + LevelFourGame.kingDoorEntryYOffset,
    );
  ...
  void update(double dt) {
    ...
    final double feetOffset =
        4 + LevelFourGame.kingGroundOffset; // Move visual stand height.
...
// Game component for the door four object.
class DoorFour extends SpriteAnimationComponent with HasGameRef<LevelFourGame> {
  final VoidCallback? onOpened;

  DoorFour({super.position, this.onOpened})
...
// Game component for the end door four object.
class EndDoorFour extends SpriteAnimationComponent
    with HasGameRef<LevelFourGame> {
  bool entering = false;
  late SpriteAnimation idleAnim;
  late SpriteAnimation openingAnim;

  EndDoorFour({super.position})
  ...
  // Opens and take king.
  Future<void> openAndTakeKing(PlayerFour king) async {
    if (entering) return;
    entering = true;
...
// Game component for the heart pickup four object.
class HeartPickupFour extends SpriteComponent
    with HasGameRef<LevelFourGame>, CollisionCallbacks {
  bool consumed = false;
  double pulseTimer = 0;
  double blinkTimer = 0;

  HeartPickupFour({super.position})
...
class BoxPig extends SpriteAnimationComponent
  ...
  Future<void> onLoad() async {
    add(RectangleHitbox());
    add(PigHealthBarFour(getHealth: () => health, maxHealth: maxHealth));
...
class BoxPiece extends SpriteComponent with HasGameRef<LevelFourGame> {
  Vector2 velocity;
  final int index;
...
class Cannon extends SpriteAnimationComponent with HasGameRef<LevelFourGame> {
...
// Game component for the king health bar four object.
class KingHealthBarFour extends PositionComponent with ParentIsA<PlayerFour> {
  KingHealthBarFour()
...
// Game component for the pig health bar four object.
class PigHealthBarFour extends PositionComponent
    with ParentIsA<PositionComponent> {
  final num Function() getHealth;
  final num maxHealth;

  PigHealthBarFour({required this.getHealth, required this.maxHealth})
