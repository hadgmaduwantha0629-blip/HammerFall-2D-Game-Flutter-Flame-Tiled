...
// Main widget for the level five scene section.
class LevelFiveScene extends StatefulWidget {
  const LevelFiveScene({super.key});

  @override
  // Creates the state object for this widget.
  State<LevelFiveScene> createState() => _LevelFiveSceneState();
}

// State for the level five scene widget.
class _LevelFiveSceneState extends State<LevelFiveScene> {
  late final LevelFiveGame game;
  ...
  Future<void> _restartLevel() async {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(builder: (_) => const LevelFiveScene()),
    );
  }
        ...
        children: [
          Positioned.fill(
            child: GameWidget(
              game: game,
              overlayBuilderMap: {
                'LevelHint': (context, LevelFiveGame game) =>
                    _buildLevelHint(game),
                'SuperPowerHud': (context, LevelFiveGame game) =>
                    _buildSuperPowerHud(game),
                'VictoryMenu': (context, LevelFiveGame game) => Center(
                  child: Container(
                    width: 360,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text(
                          'Final mission complete. The King defeated the pig kings and reclaimed the crown.',
                ...
                'DefeatMenu': (context, LevelFiveGame game) => Center(
                  child: Container(
                    width: 360,
                    padding: const EdgeInsets.all(30),
                    decoration: BoxDecoration(
  ...
  Widget _buildLevelHint(LevelFiveGame game) {
  ...
  Widget _buildPigKillHud(LevelFiveGame game) {
    return IgnorePointer(
  ...
  Widget _buildSuperPowerHud(LevelFiveGame game) {
    return IgnorePointer(
      child: SafeArea(
        child: Align(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            ListTile(
                              ...
                              splashColor: Colors.white24,
                              onTap: () {
                                LevelProgress.clearCheckpointForLevel(5);
                                Navigator.pop(context);
                                _restartLevel();
                              },
                            ),
...
// Main game logic for the level five game scene.
class LevelFiveGame extends FlameGame with HasCollisionDetection {
  static const double _tileSize = 32.0;
  static const double levelZoom = 1.8;
  static const double cameraFollowMaxSpeed = 1200.0;
  ...
  late PlayerFive player;
  EndDoorFive? endDoor;
  List<Rect> groundRects = [];
  double mapWidth = 0;
  double mapHeight = 0;
  int pigKills = 0;
  final int requiredPigKills = 20;
  final ValueNotifier<int> pigKillsNotifier = ValueNotifier<int>(0);
  Vector2? kingPigOneSpawnPoint;
  double kingPigOneSpawnTimer = 0;
  bool kingPigTwoDefeated = false;
  ...
  Future<void> onLoad() async {
    ...
    if (spawns != null) {
      ...
      final useResume = LevelProgress.consumeResumeChoiceForLaunch(level: 5);
      final resume = LevelProgress.resumePositionForLevel(5);
      if (useResume && resume != null) {
        player.position.setValues(resume.x, resume.y);
        showLevelHint('Resumed from your last checkpoint.');
      }
      LevelProgress.setLastPlayedLevel(5);
      ...
      for (final obj in spawns.objects) {
        ...
        if (objName == 'startdoor') {
          ...
          endDoor = EndDoorFive(position: Vector2(obj.x, obj.y));
          world.add(endDoor!);
        ...
        } else if (objName == 'king_pig_1') {
          kingPigOneSpawnPoint = Vector2(obj.x, obj.y);
        } else if (objName == 'king_pig_2') {
          world.add(KingPigTwo(position: Vector2(obj.x, obj.y)));
        } else if (objName == 'heart') {
          world.add(HeartPickupFive(position: Vector2(obj.x, obj.y)));
    ...
    camera.follow(player, maxSpeed: cameraFollowMaxSpeed, snap: false);
  ...
  void update(double dt) {
    ...
    if (!gameEnded && !endSequenceStarted && kingPigOneSpawnPoint != null) {
      kingPigOneSpawnTimer += dt;
      while (kingPigOneSpawnTimer >= 10) {
        kingPigOneSpawnTimer -= 10;
        world.add(GiantPig(position: kingPigOneSpawnPoint!.clone()));
      }
    }

    if (gameEnded || endSequenceStarted || endDoor == null) return;
    if (player.isEntering || player.isEnteringDoor) return;
    if (!kingPigTwoDefeated) return;
  ...
  // Handles on king pig two killed.
  void onKingPigTwoKilled() {
    if (gameEnded) return;
    kingPigTwoDefeated = true;
    if (pigKills < requiredPigKills) {
      final remaining = requiredPigKills - pigKills;
      showLevelHint('King pig defeated. Kill $remaining more pigs to win.');
    }
  }
...
// Game component for the player five object.
class PlayerFive extends SpriteAnimationComponent
    with HasGameRef<LevelFiveGame>, CollisionCallbacks {
  ...
  // Handles enter door.
  Future<void> enterDoor(EndDoorFive door) async {
    if (isDead || isEntering || isEnteringDoor) return;
    isEnteringDoor = true;
    horizontalMovement = 0;
    velocity = Vector2.zero();
    position.setValues(
      door.position.x,
      door.position.y + LevelFiveGame.kingDoorEntryYOffset,
    );
  ...
  void update(double dt) {
    ...
    final double feetOffset =
        4 + LevelFiveGame.kingGroundOffset; // Move visual stand height.
...
// Game component for the door five object.
class DoorFive extends SpriteAnimationComponent with HasGameRef<LevelFiveGame> {
  final VoidCallback? onOpened;

  DoorFive({super.position, this.onOpened})
...
// Game component for the end door five object.
class EndDoorFive extends SpriteAnimationComponent
    with HasGameRef<LevelFiveGame> {
  bool entering = false;
  late SpriteAnimation idleAnim;
  late SpriteAnimation openingAnim;

  EndDoorFive({super.position})
  ...
  // Opens and take king.
  Future<void> openAndTakeKing(PlayerFive king) async {
    if (entering) return;
    entering = true;
...
// Game component for the heart pickup five object.
class HeartPickupFive extends SpriteComponent
    with HasGameRef<LevelFiveGame>, CollisionCallbacks {
  bool consumed = false;
  double pulseTimer = 0;
  double blinkTimer = 0;

  HeartPickupFive({super.position})
...
class BoxPig extends SpriteAnimationComponent
  ...
  Future<void> onLoad() async {
    add(RectangleHitbox());
    add(PigHealthBarFive(getHealth: () => health, maxHealth: maxHealth));
...
class BoxPiece extends SpriteComponent with HasGameRef<LevelFiveGame> {
  Vector2 velocity;
  final int index;
...
class Cannon extends SpriteAnimationComponent with HasGameRef<LevelFiveGame> {
...
// Class for the king pig two feature.
class KingPigTwo extends GiantPig {
  KingPigTwo({super.position});

  @override
  // Loads assets and child components.
  Future<void> onLoad() async {
    await super.onLoad();
    health = GameSettings.pigHealthDouble(10);
  }

  @override
  // Reduces health when this object gets hit.
  void takeDamage({double amount = 1}) {
    final wasDead = isDead;
    super.takeDamage(amount: amount);
    if (!wasDead && isDead) {
      gameRef.onKingPigTwoKilled();
    }
  }
}
...
// Game component for the king health bar five object.
class KingHealthBarFive extends PositionComponent with ParentIsA<PlayerFive> {
  KingHealthBarFive()
...
// Game component for the pig health bar five object.
class PigHealthBarFive extends PositionComponent
    with ParentIsA<PositionComponent> {
  final num Function() getHealth;
  final num maxHealth;

  PigHealthBarFive({required this.getHealth, required this.maxHealth})
